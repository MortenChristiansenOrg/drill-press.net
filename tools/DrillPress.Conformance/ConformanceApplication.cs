...
public sealed class ConformanceApplication(
    IFileSystem fileSystem,
    MsBuildSnapshotLoader loader,
    AnalysisEngine engine,
    CompilationSnapshotFile storage
)
{
    ...
    public async Task<ConformanceExitCode> RunAsync(
        string[] args,
    ...
    )
    {
        ...
        try
        {
            var export = await _loader.LoadCompilationsAsync(
                target,
            ...
            try
            {
                ...
                var comparisons = export
                    .Contexts.Zip(reconstructed)
                ...
                var ruleParity = BundleResponseProtocol
                    .Serialize(liveRules)
                    .AsSpan()
                    .SequenceEqual(BundleResponseProtocol.Serialize(restoredRules));
                var report = new
                {
                    target = displayTarget ?? target,
                    sdk = export.Contexts.Select(context => context.Snapshot.SdkVersion).Distinct(),
                    snapshotBytes = _fileSystem.FileInfo.New(path).Length,
                    ruleParity,
                    rules = liveRules
                        ...
                        .Select(group => new
                        {
                            ...
                            fixable = group.Count(finding => finding.BatchId is not null),
                        }),
                    proposedBatches = liveRules.Batches.Length,
                    contexts = comparisons,
                };
                var fullReport = _fileSystem.Path.GetFullPath(reportPath);
                _fileSystem.Directory.CreateDirectory(
                    _fileSystem.Path.GetDirectoryName(fullReport)!
                );
                ...
                return ruleParity && comparisons.All(comparison => comparison.Equal)
                    ? ConformanceExitCode.Equal
                    : ConformanceExitCode.Mismatch;
            }
            finally
            {
        ...
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
            ...
            await _fileSystem.File.WriteAllTextAsync(
                fullReport,
                JsonSerializer.Serialize(
                    new { target = displayTarget ?? target, error = exception.ToString() }
                ),
                cancellationToken
            );
    ...
    private static ContextComparison Compare(
        CompilationContext live,
        CompilationContext reconstructed,
    ...
    )
    {
        ...
        return new ContextComparison(
            live.Snapshot.Name,
            live.Snapshot.TargetFramework,
            expected.Length,
            expected.SequenceEqual(actual),
            expected.Except(actual).Take(10).ToArray(),
            actual.Except(expected).Take(10).ToArray()
        );
    }

    private sealed record ContextComparison(
        string Project,
        string Framework,
        int Probes,
        bool Equal,
