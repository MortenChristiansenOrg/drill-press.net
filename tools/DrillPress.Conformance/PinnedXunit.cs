    ...
    )
    {
        ...
        try
        {
            if (!_fileSystem.Directory.Exists(directory))
            {
                ...
                await Run(
                    "git",
                    ["clone", "https://github.com/xunit/xunit.git", directory],
                    parent
                );
            ...
            if (head.Trim() != Revision)
            {
                throw new InvalidOperationException(
                    $"Expected xUnit revision {Revision}; use a new checkout directory."
                );
            }
            ...
            {
                throw new InvalidOperationException(
                    "The pinned xUnit checkout has tracked modifications; use a clean checkout."
                );
            }
            ...
            await Run(
                ...
                [
                    ...
                    "--use-lock-file",
                    "--lock-file-path",
                    "obj/drillpress.packages.lock.json",
                    "-p:RestoreLockedMode=false",
                    "--nologo",
                ],
                directory
            );
            var lockDirectory = _fileSystem.Path.Combine(
                _fileSystem.Path.GetDirectoryName(reportPath)!,
                "dependency-locks"
            );
            foreach (
                var path in _fileSystem.Directory.EnumerateFiles(
                    directory,
                    "drillpress.packages.lock.json",
                    SearchOption.AllDirectories
                )
            )
            {
                var copy = _fileSystem.Path.Combine(
                    lockDirectory,
                    _fileSystem.Path.GetRelativePath(directory, path)
                );
