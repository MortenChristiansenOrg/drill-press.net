...
internal sealed class RuleCoverageCase(IFileSystem fileSystem, string root, string directory)
{
    ...
    public async Task<BundleCase> CreateAsync(string buildHost)
    {
        ...
        ProcessRunner.RequireSuccess(
            await ProcessRunner.RunAsync("dotnet", ["restore", projectPath, "--nologo"], root)
        );
        ProcessRunner.RequireSuccess(
            await ProcessRunner.RunAsync(
                "dotnet",
                [buildHost, "export", projectPath, snapshotPath, "--validate-compilation"],
                root
            )
        );
        var snapshot = await new CompilationSnapshotFile(_fileSystem).ReadAsync(snapshotPath);
        var response = ExpectedResponse(snapshot);
        return new BundleCase(
            "all-rules",
            ["check", snapshotPath],
    ...
    private async Task<string> WriteProjectAsync()
    {
        ...
        var referenceDirectory = _fileSystem.Path.Combine(
            root,
            "tests",
        ...
        var project = new XElement(
            ...
            new XElement(
                ...
                new XElement("IsTestProject", "false")
            ),
            new XElement(
                "ItemGroup",
                new[] { "xunit.v3.core", "xunit.v3.assert" }.Select(name => new XElement(
                    "Reference",
                    new XAttribute("Include", name),
                    new XElement(
                        "HintPath",
                        _fileSystem.Path.Combine(referenceDirectory, name + ".dll")
                    )
                ))
            )
        );
        ...
        await _fileSystem.File.WriteAllTextAsync(
            _fileSystem.Path.Combine(directory, "Coverage.cs"),
            _source
        );
        return projectPath;
    }
    ...
    private BundleResponse ExpectedResponse(CompilationSnapshot snapshot)
    {
        ...
        var document = project.Documents.Single(document =>
            document.Path == _fileSystem.Path.Combine(directory, "Coverage.cs")
        );
        ...
        [
            new(
                ...
                document.DocumentId,
                _source.IndexOf("\n\n        Assert.Single", StringComparison.Ordinal) + 1,
                0,
                null
            ),
            Find(
    ...
    private static Finding Find(
        ...
        new(
            ...
            message,
            document.DocumentId,
            _source.IndexOf(text, StringComparison.Ordinal),
            text.Length,
            null
        );

    private static FixBatch ExpectedFix(
        DocumentSnapshot document,
        string original,
        string replacement,
        string contextId
    )
    {
        var edit = new SourceEdit(
            ...
            _source.IndexOf(original, StringComparison.Ordinal),
            original.Length,
            original,
            replacement
        );
