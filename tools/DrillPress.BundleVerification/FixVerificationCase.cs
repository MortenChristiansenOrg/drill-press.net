...
internal sealed class FixVerificationCase(
...
)
{
    ...
    )
    {
        ...
        [
            ...
            .. Encoding.Unicode.GetBytes(originalFirst.Replace("string.Empty", "\"\"")),
        ];
        var expectedSecond = Encoding.UTF8.GetBytes(
            originalSecond.Replace("StringComparer.Ordinal", "")
        );
        foreach (var mode in Enum.GetValues<BundleMode>())
        {
            ...
            var result = await ProcessRunner.RunAsync(
                ...
                [
                    ...
                    _fileSystem.Path.Combine(directory, "*.cs"),
                ],
                root
            );
            await _fileSystem.File.WriteAllBytesAsync(
                _fileSystem.Path.Combine(output, $"fix.{mode}.stdout"),
                result.StandardOutput
            );
            await _fileSystem.File.WriteAllBytesAsync(
                _fileSystem.Path.Combine(output, $"fix.{mode}.stderr"),
            ...
            if (
                ...
                || !(await _fileSystem.File.ReadAllBytesAsync(first))
                    .AsSpan()
                    .SequenceEqual(expectedFirst)
                || !(await _fileSystem.File.ReadAllBytesAsync(second))
                    .AsSpan()
                    .SequenceEqual(expectedSecond)
                || _fileSystem.Directory.GetFiles(directory).Length != 2
            )
            {
                throw new InvalidDataException(
                    $"{mode} fix/recheck did not produce the exact clean sources and output."
                );
            }
        }
    }
