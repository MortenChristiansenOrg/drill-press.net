...
public sealed class VerificationSession : IDisposable
{
    ...
    public string ManagedBundle =>
        _fileSystem.Path.Combine(OutputDirectory, "managed", "DrillPress.SampleRules.dll");
    public string NativeBundle =>
    ...
    public static async Task<VerificationSession> CreateAsync(
    ...
    )
    {
        ...
        var root =
            directory?.FullName
            ?? throw new InvalidOperationException("Run from the repository directory.");
        if (
        ...
        )
        {
            throw new PlatformNotSupportedException(
                "Supported runtime identifiers are linux-x64 and win-x64."
            );
        }

        var rid = OperatingSystem.IsWindows() ? "win-x64" : "linux-x64";
        output = fileSystem.Path.GetFullPath(
            output ?? fileSystem.Path.Combine(root, "artifacts", $"native-{rid}-{Guid.NewGuid():N}")
        );
    ...
    private async Task PublishAsync()
    {
        ...
        await BuildCommandAsync(
            ...
            [
                ...
                "false",
                "-o",
                _fileSystem.Path.Combine(OutputDirectory, "managed"),
            ]
        );
        var publication = await BuildCommandAsync(
            "publish.log",
            [
                "publish",
                sample,
                ...
                RuntimeIdentifier,
                "-o",
                _fileSystem.Path.Combine(OutputDirectory, "native"),
            ]
        );
    ...
    private async Task<ProcessOutput> BuildCommandAsync(string log, string[] arguments)
    {
        ...
        await _fileSystem.File.WriteAllBytesAsync(
            _fileSystem.Path.Combine(OutputDirectory, log),
    ...
    private string BuildHost =>
        _fileSystem.Path.Combine(
            RepositoryRoot,
            "src/DrillPress.BuildHost/bin/Release/net10.0/DrillPress.BuildHost.dll"
        );
        ...
        )
        {
            ...
            ProcessRunner.RequireSuccess(
                await ProcessRunner.RunAsync(
                    "dotnet",
                    [BuildHost, "export", project, snapshot],
                    RepositoryRoot
                )
            );
            ...
            PublicOutput = Encoding.UTF8.GetBytes(
                new CompactDiagnosticRenderer(_fileSystem).Render(
                    new BundleResponseValidator().Validate(captured, response)
                )
            );
    ...
    private async Task CreateContextCasesAsync()
    {
        ...
        var baseline = await storage.ReadAsync(
            _fileSystem.Path.Combine(_fixture.FullName, "violating.json")
        );
        var project = baseline.Projects.Single();
        var ordinary = project.Documents.Single(document =>
        ...
        var cases = new List<BundleCase>();
        foreach (
            var (name, symbols) in new[]
            {
                ("linked", new[] { "INCLUDED" }),
                ("inactive", Array.Empty<string>()),
            }
        )
        {
            var second = first with
            {
                ...
                Documents = [first.Documents[0] with { DocumentId = "second-doc" }],
            };
            ...
            var response = new BundleResponse(
                ...
                [
                    ...
                    new(
                        "second",
                        true,
                        symbols.Length == 0 ? [] : [finding with { DocumentId = "second-doc" }]
                    ),
                ],
                []
            );
            cases.Add(
                new BundleCase(
                    ...
                    BundleResponseProtocol.Serialize(response),
                    []
                )
            );
        }
    ...
    private async Task VerifyAsync()
    {
        foreach (var @case in Cases.Concat(_contextCases))
        {
            ...
            Console.WriteLine(
                $"{@case.Name}: managed/native bytes and exit code {(int)@case.Outcome} match"
            );
        }

        var cli = _fileSystem.Path.Combine(
            RepositoryRoot,
            "src/DrillPress.Cli/bin/Release/net10.0/DrillPress.Cli.dll"
        );
        var resultCli = await ProcessRunner.RunAsync(
            "dotnet",
            [
                cli,
                "check",
        ...
        BundleContract.Validate(
            Cases.Single(@case => @case.Name == "violating") with
            {
        ...
        await _fileSystem.File.WriteAllBytesAsync(
            _fileSystem.Path.Combine(OutputDirectory, "public.stdout"),
            PublicOutput
        );
        ...
        var coverage = await new RuleCoverageCase(
            _fileSystem,
            RepositoryRoot,
            _fileSystem.Path.Combine(_fixture.FullName, "Coverage")
        ).CreateAsync(BuildHost);
        foreach (var mode in Enum.GetValues<BundleMode>())
        {
            var result = await ExecuteAsync(mode, coverage);
            await _fileSystem.File.WriteAllBytesAsync(
                _fileSystem.Path.Combine(OutputDirectory, $"all-rules.{mode}.stdout"),
                result.StandardOutput
            );
        }
        ...
        await new FixVerificationCase(
            _fileSystem,
            RepositoryRoot,
            _fileSystem.Path.Combine(_fixture.FullName, "FixTargets"),
            OutputDirectory
