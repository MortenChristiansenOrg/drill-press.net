...
public sealed class ResultSignatures(IFileSystem fileSystem)
{
    ...
    public async Task<ResultSignatureFiles> WriteAsync(
        string snapshotPath,
        string responsePath,
        string prefix,
        CancellationToken cancellationToken,
        string? normalizationRoot = null
    )
    {
        var snapshot = await new CompilationSnapshotFile(_fileSystem).ReadAsync(
            snapshotPath,
            cancellationToken
        );
        var responseBytes = await _fileSystem.File.ReadAllBytesAsync(
            responsePath,
            cancellationToken
        );
        ...
        var publicBytes = Encoding.UTF8.GetBytes(
            new CompactDiagnosticRenderer(_fileSystem).Render(plan)
        );
        prefix = _fileSystem.Path.GetFullPath(prefix);
        _fileSystem.Directory.CreateDirectory(_fileSystem.Path.GetDirectoryName(prefix)!);
        var files = new ResultSignatureFiles(
            prefix + ".response.json",
            prefix + ".plan.json",
            prefix + ".public.stdout",
            Hash(responseBytes),
            Hash(planBytes),
            Hash(publicBytes),
        ...
        await _fileSystem.File.WriteAllBytesAsync(
            files.PublicOutputPath,
            publicBytes,
            cancellationToken
        );
        await _fileSystem.File.WriteAllTextAsync(
            prefix + ".signature.json",
    ...
    {
        if (
            expected.ResponseHash != actual.ResponseHash
            || expected.PlanHash != actual.PlanHash
            || expected.PublicOutputHash != actual.PublicOutputHash
        )
        {
            throw new InvalidDataException(
                $"Complete diagnostic/fix signatures differ: {expected.ResponsePath} and {actual.ResponsePath}."
            );
        }
    }
