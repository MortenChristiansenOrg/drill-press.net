...
public sealed class SignatureNormalization
{
    ...
    public SignatureNormalization(IFileSystem fileSystem, CompilationSnapshot snapshot, string root)
    {
        ...
        _contexts = snapshot.Projects.ToDictionary(
            project => project.ContextId,
            project =>
                Hash(
                    new
                    {
        ...
        _paths = snapshot
            .Projects.SelectMany(project =>
                project.Documents.Select(document =>
                    (document.Path, Normalized: DocumentPath(project, document))
                )
            )
            .Distinct()
                ...
                project.Documents.Select(document =>
                    (
                        ...
                        Identity: Hash(
                            new
                            {
                                Context = _contexts[project.ContextId],
                                Path = _paths[document.Path],
                            }
                        )
                    )
                )
        ...
        if (
            _contexts.Values.Distinct().Count() != _contexts.Count
            || _documents.Values.Distinct().Count() != _documents.Count
        )
        {
            throw new InvalidDataException(
                "Benchmark contexts or document memberships do not have unique normalized identities."
            );
        }
    ...
    public CompilationSnapshot Snapshot(CompilationSnapshot snapshot) =>
        snapshot with
        {
            RequestId = "request",
            Projects = snapshot
                .Projects.Select(project =>
                    project with
                    {
                        ...
                        {
                            SpecificDiagnosticOptions = Sort(
                                project.CompilerOptions.SpecificDiagnosticOptions
                            ),
                        },
                        ReferencedContextIds = project
                            .ReferencedContextIds.Select(id => _contexts[id])
                            .ToArray(),
                        CompilationReferences = project
                            .CompilationReferences.Select(reference =>
                                reference with
                                {
                                    ContextId = _contexts[reference.ContextId],
                                }
                            )
                            .ToArray(),
                        MetadataReferences = project.MetadataReferences.Select(Path).ToArray(),
                        ExternalReferences = project
                            .ExternalReferences.Select(reference =>
                                reference with
                                {
                        ...
                        Documents = project
                            .Documents.Select(document =>
                                document with
                                {
                                    DocumentId = _documents[document.DocumentId],
                                    FileIdentity = _files[document.FileIdentity],
                                    Path = _paths[document.Path],
                                    Options = document.Options is null
                                        ? null
                                        : document.Options with
                                        {
                                            Features = Sort(document.Options.Features),
                                            DiagnosticOptions = Sort(
                                                document.Options.DiagnosticOptions
                                            ),
                                        },
                                }
                            )
                    ...
                    context with
                    {
                        ...
                        Findings = context
                            .Findings.Select(finding =>
                                finding with
                                {
                                    DocumentId = _documents[finding.DocumentId],
                                    BatchId = BatchId(finding.BatchId),
                                }
                            )
                            .ToArray(),
                    }
        ...
        return plan with
        {
            Findings = plan
                .Findings.Select(finding =>
                    finding with
                    {
                        FileIdentity = _files[finding.FileIdentity],
                ...
                .ToArray(),
            Batches = batches,
            Edits = plan.Edits.Select(Edit).ToArray(),
        };
    }
    ...
    private FixBatch Batch(FixBatch batch)
    {
        ...
        var validations = batch
            .Validations.Select(validation =>
                validation with
                {
                    ContextId = _contexts[validation.ContextId],
                }
            )
            .ToArray();
    ...
    private SourceEdit Edit(SourceEdit edit) =>
        edit with
        {
            FileIdentity = _files[edit.FileIdentity],
        };
    ...
    private string DocumentPath(ProjectSnapshot project, DocumentSnapshot document)
    {
        ...
        return document.IsGenerated && document.Path.StartsWith(prefix, StringComparison.Ordinal)
            ? "drillpress-generated://"
                + _contexts[project.ContextId]
                + "/"
                + document.Path[prefix.Length..]
            : Path(document.Path);
    }
