...
public sealed class RepositoryMeasurements(
...
)
{
    ...
    public async Task<MeasuredExecution> RunAsync(
        string name,
        string executable,
        string[] arguments,
        string workingDirectory,
        bool findingsAllowed,
        string[] profileComponents,
        CancellationToken cancellationToken
    )
    {
        Console.Error.WriteLine($"performance: {name}");
        var execution = await _process.RunAsync(
            executable,
            arguments,
            workingDirectory,
            _fileSystem.Path.Combine(_output, name),
            cancellationToken
        );
        ...
        if (execution.ExitCode != 0 && !(findingsAllowed && execution.ExitCode == 1))
        {
            throw new InvalidOperationException(
                $"{name} exited {execution.ExitCode}; see {execution.StandardErrorPath} and {execution.StandardOutputPath}."
            );
        }
        ...
        if (profileComponents.Length > 0)
        {
            _operations[^1] = new(
                ...
                _profiles.Read(
                    ...
                    execution.Measurement.WallMilliseconds,
                    profileComponents
                )
            );
            await SaveAsync();
    ...
    private Task SaveAsync() =>
        _fileSystem.File.WriteAllTextAsync(
            _fileSystem.Path.Combine(_output, "operations.json"),
    ...
    public Task<MeasuredExecution> DotnetAsync(
        string name,
        string[] arguments,
        CancellationToken cancellationToken
