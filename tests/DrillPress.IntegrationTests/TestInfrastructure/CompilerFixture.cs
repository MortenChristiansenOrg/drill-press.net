...
public sealed class CompilerFixture : IntegrationTest, IAsyncLifetime
{
    ...
    public string Consumer { get; } = RepositoryPath("fixtures", "CompilerSnapshot", "Consumer", "Consumer.csproj");

    public async ValueTask InitializeAsync()
    {
        foreach (var project in new[] { "Generator", "Interop" })
        {
            var result = await RunProcessAsync("dotnet", ["build", RepositoryPath("fixtures", "CompilerSnapshot", project, project + ".csproj"), "-c", "Release"],
