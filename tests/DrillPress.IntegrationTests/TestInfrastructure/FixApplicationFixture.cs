...
internal sealed class FixApplicationFixture : IntegrationTest
{
    ...
    public FixApplicationFixture()
    {
        ...
        Paths = new[] { "A.cs", "B.cs", "C.cs" }.Select(name => FileSystem.Path.Combine(DirectoryPath, name)).ToArray();
        foreach (var path in Paths) FileSystem.File.WriteAllBytes(path, Encoding.UTF8.GetBytes(Original));
        var documents = Paths.Select(path => SourceIdentity.Capture(new DocumentSnapshot(path, Original, false),
            FileSystem.File.ReadAllBytes(path), "utf-8", false)).ToArray();
        var project = new ProjectSnapshot("Fixture", "Fixture", FileSystem.Path.Combine(DirectoryPath, "Fixture.csproj"),
            (int)LanguageVersion.CSharp14, (int)OutputKind.DynamicallyLinkedLibrary, (int)NullableContextOptions.Enable, [], documents, []);
        Snapshot = CompilationSnapshot.Create(project);
        var batches = documents.Select((document, index) => new FixBatch($"fix{index}",
            [new(document.FileIdentity, document.Fingerprint, 25, 12, "string.Empty", "\"\"")], [new(project.ContextId, true)])).ToArray();
        Response = new(1, Snapshot.RequestId,
            [new(project.ContextId, true, documents.Select((document, index) => new Finding("DP1004", "Use literal.", document.DocumentId, 25, 12, $"fix{index}")).ToArray())], batches);
