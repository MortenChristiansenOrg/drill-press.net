...
public sealed class SnapshotPermissionFixture : IntegrationTest
{
    ...
    {
        var ready = FileSystem.Path.Combine(
            CreateTemporaryDirectory("drillpress-permissions-").FullName,
            "ready"
        );
        ...
        var snapshot = await FileSystem.File.ReadAllTextAsync(
            ready + ".snapshot",
            TestContext.Current.CancellationToken
        );
        ...
        try
        {
            await new CompilationSnapshotFile().WriteAsync(
                snapshot,
                CompilationSnapshot.Create(),
                TestContext.Current.CancellationToken
            );
