...
internal sealed class ReplacementFailureProbe(
...
) : AtomicFileReplacer(fileSystem, policy)
{
    ...
    )
    {
        ...
        if (FailPreparation && file.Path == failurePath)
            throw new IOException("injected preparation failure");
    }

    internal override Task ReplaceAsync(
    ...
    )
    {
        if (file.Path == failurePath)
        {
            ...
            if (ConcurrentEdit)
                _fileSystem.File.WriteAllText(file.Path, "external edit");
        }
