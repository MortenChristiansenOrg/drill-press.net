...
public sealed class SemanticRuleFixture
{
    ...
    public SemanticRuleFixture()
    {
        ...
        _references = ((string)AppContext.GetData("TRUSTED_PLATFORM_ASSEMBLIES")!)
    ...
    public AnalysisProject Project(
        string source,
        string name = "Test",
        bool isTest = false,
        string framework = "net10.0",
        string[]? symbols = null,
        AnalysisProject[]? dependencies = null,
        DocumentSnapshot[]? extra = null,
        bool allowErrors = false
    )
    {
        dependencies ??= [];
        var document = SourceIdentity.Capture(
            new DocumentSnapshot(name + ".cs", source, false),
        ...
        var compilation = CSharpCompilation.Create(
            name,
            documents.Select(item => CSharpSyntaxTree.ParseText(item.Text, options, item.Path)),
            _references.Concat(
                dependencies.Select(dependency => dependency.Compilation.ToMetadataReference())
            ),
        ...
        )
        {
            ...
            ReferencedContextIds = dependencies
                .Select(dependency => dependency.Snapshot.ContextId)
                .ToArray(),
        };
    ...
    public async Task<(string Rule, int Line, string Text, string? Replacement)[]> Describe(
        params AnalysisProject[] projects
    )
    {
        ...
        return response
            .Contexts.SelectMany(context =>
                ...
                {
                    var project = projects.Single(project =>
                        project.Snapshot.ContextId == context.ContextId
                    );
                    var source = project.Sources.Single(source =>
                        source.Document.DocumentId == finding.DocumentId
                    );
                    return (
                        finding.RuleId,
                        source.Locate(new(finding.Start, finding.Length)).Line,
                        source.Document.Text.Substring(finding.Start, finding.Length),
                        response
                            .Batches.SingleOrDefault(batch => batch.Id == finding.BatchId)
                            ?.Edits.Single()
                            .Replacement
                    );
                })
