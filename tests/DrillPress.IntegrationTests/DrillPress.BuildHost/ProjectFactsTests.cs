...
public sealed class ProjectFactsTests : IntegrationTest
{
    ...
    public async Task Project_capture_preserves_direct_central_packages_and_policy_properties()
    {
        ...
        var packages = XDocument.Parse(FileSystem.File.ReadAllText(RepositoryPath("Directory.Packages.props")));
        var version = packages.Descendants("PackageVersion").Single(item => item.Attribute("Include")!.Value == "Microsoft.NET.StringTools").Attribute("Version")!.Value;
        FileSystem.File.WriteAllText(FileSystem.Path.Combine(root, "Directory.Packages.props"), $$"""
            ...
            """);
        FileSystem.File.WriteAllText(target, """
        ...
        var exported = await RunProcessAsync("dotnet", [GetOutputPath("DrillPress.BuildHost"), "export", target, output],
