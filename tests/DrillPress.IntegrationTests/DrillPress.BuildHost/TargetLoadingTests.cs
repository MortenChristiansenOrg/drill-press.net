...
public sealed class TargetLoadingTests : IntegrationTest
{
    ...
    public async Task Quoted_glob_is_ordinal_sorted_and_uses_loose_mode_inside_a_project()
    {
        ...
        await WriteAsync(root, "Unrestored.csproj", "<Project Sdk=\"Microsoft.NET.Sdk\"><PropertyGroup><TargetFramework>net10.0</TargetFramework></PropertyGroup></Project>");
        ...
        Assert.Equal([FileSystem.Path.Combine(root, "Z.cs"), FileSystem.Path.Combine(root, "nested", "A.cs")], project.Documents.Select(document => document.Path));
    ...
    public async Task Zero_match_glob_fails_without_creating_a_snapshot()
    {
        ...
        Assert.Equal(new ProcessResult(2, "", $"DrillPress.BuildHost: C# glob '{target}' matched no files.{Environment.NewLine}"), result);
    ...
    public async Task Missing_restore_assets_fail_without_an_implicit_restore()
    {
        ...
        await WriteAsync(root, "Target.csproj", "<Project Sdk=\"Microsoft.NET.Sdk\"><PropertyGroup><TargetFramework>net10.0</TargetFramework></PropertyGroup></Project>");
    ...
    public async Task Fast_mode_preserves_an_erroneous_source_dependency_and_validation_rejects_it()
    {
        ...
        await WriteAsync(root, "dependency/Dependency.csproj", "<Project Sdk=\"Microsoft.NET.Sdk\"><PropertyGroup><TargetFramework>net10.0</TargetFramework></PropertyGroup></Project>");
        await WriteAsync(root, "dependency/Source.cs", "public class Dependency { public static string Value => string.Empty; public Missing Error; }");
        await WriteAsync(root, "consumer/Consumer.csproj", """
            ...
            """);
        await WriteAsync(root, "consumer/Source.cs", "extern alias dep; class Consumer { string Value => dep::Dependency.Value; }");
        await WriteAsync(root, "Selected.slnx", "<Solution><Project Path=\"consumer/Consumer.csproj\" /></Solution>");
        ...
        var validated = await ExportAsync(target, FileSystem.Path.Combine(root, "validated.json"), "--validate-compilation");
        ...
        Assert.Empty(consumer.Compilation.GetDiagnostics(TestContext.Current.CancellationToken).Where(diagnostic => diagnostic.Severity == Microsoft.CodeAnalysis.DiagnosticSeverity.Error));
    ...
    public async Task Target_global_json_is_honored_from_an_unrelated_working_directory()
    {
        ...
        await WriteAsync(root, "global.json", """{"sdk":{"version":"99.0.100","rollForward":"disable"}}""");
        await WriteAsync(root, "Target.csproj", "<Project Sdk=\"Microsoft.NET.Sdk\"><PropertyGroup><TargetFramework>net10.0</TargetFramework></PropertyGroup></Project>");
        var output = FileSystem.Path.Combine(root, "snapshot.json");
    ...
    public async Task Loose_source_preserves_utf16_bytes_and_bom()
    {
        ...
        byte[] bytes = [.. System.Text.Encoding.Unicode.GetPreamble(), .. System.Text.Encoding.Unicode.GetBytes(text)];
        await FileSystem.File.WriteAllBytesAsync(source, bytes, TestContext.Current.CancellationToken);
        var output = FileSystem.Path.Combine(root, "snapshot.json");
    ...
    private Task<ProcessResult> ExportAsync(string target, string output, params string[] options) =>
        RunProcessAsync("dotnet", [GetOutputPath("DrillPress.BuildHost"), "export", target, output, .. options], RepositoryRoot, TestContext.Current.CancellationToken);
}
