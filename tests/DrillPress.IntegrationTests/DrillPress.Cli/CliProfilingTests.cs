...
public sealed class CliProfilingTests : IntegrationTest
{
    ...
    public async Task Profile_reports_complete_process_phases_without_changing_compact_stdout()
    {
        ...
        var plain = await RunProcessAsync(
            "dotnet",
            arguments,
            RepositoryRoot,
            TestContext.Current.CancellationToken
        );
        var profiled = await RunProcessAsync(
            "dotnet",
            [.. arguments, "--profile", "--no-optimization"],
            RepositoryRoot,
            TestContext.Current.CancellationToken
        );
        ...
        Assert.Equal(
            [
                "cli.build-host",
                "build-host.loading",
                "build-host.snapshot.serialization",
                "build-host.snapshot.bytes",
                "build-host.contexts",
                "build-host.total",
                "cli.snapshot.loading",
                "cli.rules",
                "rules.snapshot.loading",
                ...
                "rules.rule.DP1004",
                "rules.rule.DP1005",
                "rules.member.symbol.bindings",
                "rules.interface.definition.comparisons",
                "rules.fix.validation",
                "rules.aggregation",
                "rules.contexts",
                "rules.context.findings",
                "rules.actionable.locations",
                "rules.common.safe.batches",
                "rules.common.safe.edits",
                "rules.response.serialization",
                "rules.total",
                "cli.public.bytes",
                "cli.rendering",
                "cli.total",
        ...
        Assert.Equal(
            1,
            events
                .Single(entry => entry.Component == "rules" && entry.Phase == "context.findings")
                .Count
        );
        Assert.Equal(
            System.Text.Encoding.UTF8.GetByteCount(plain.StandardOutput),
            events.Single(entry => entry.Phase == "public.bytes").Count
        );
        Assert.All(
            events,
            entry =>
    ...
    private static ProfileEvent[] ParseEvents(string output) =>
        output
