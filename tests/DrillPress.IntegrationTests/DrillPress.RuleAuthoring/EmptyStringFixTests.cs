...
public sealed class EmptyStringFixTests(SemanticRuleFixture fixture) : IClassFixture<SemanticRuleFixture>
{
    [Theory]
    [InlineData("string.Empty", "\"\"")]
    [InlineData("global::System.String.Empty", "\"\"")]
    [InlineData("nameof(string.Empty)", null)]
    public async Task Diagnoses_bound_aliases_and_nameof_but_only_fixes_values(string expression, string? replacement)
    {
        var project = fixture.Project($"class C {{ string Value => {expression}; }}");

        var findings = await fixture.Describe(project);

        Assert.Equal([("DP1004", 1, expression.Replace("nameof(", "").TrimEnd(')'), replacement)], findings);
    }

    [Fact]
    public async Task Expression_tree_representation_is_preserved()
    {
        var project = fixture.Project("class C { System.Linq.Expressions.Expression<System.Func<string>> Value => () => string.Empty; }");
    ...
    {
        var project = fixture.Project("class C { string Value => Missing(string.Empty); }", allowErrors: true);
    ...
    public async Task Agreeing_linked_contexts_share_one_complete_batch()
    {
        ...
        var second = fixture.Project("class C { string Value => string.Empty; }", framework: "net9.0");

        var response = await fixture.Evaluate(first, second);

        var batch = Assert.Single(response.Batches);
        Assert.Equal([first.Snapshot.ContextId, second.Snapshot.ContextId], batch.Validations.Select(validation => validation.ContextId));
        Assert.Equal([true, true], batch.Validations.Select(validation => validation.IsSafe));
        Assert.Equal([batch.Id, batch.Id], response.Contexts.SelectMany(context => context.Findings).Select(finding => finding.BatchId));
    ...
    {
        var project = fixture.Project("class C { string Value => /* before */ string.Empty /* after */; }");

        var response = await fixture.Evaluate(project);

        var edit = Assert.Single(Assert.Single(response.Batches).Edits);
        Assert.Equal("class C { string Value => /* before */ \"\" /* after */; }",
            project.Snapshot.Documents[0].Text.Remove(edit.Start, edit.Length).Insert(edit.Start, edit.Replacement));
    }
