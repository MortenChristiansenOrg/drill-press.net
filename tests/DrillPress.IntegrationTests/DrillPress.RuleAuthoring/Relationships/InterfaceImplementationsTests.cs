...
public sealed class InterfaceImplementationsTests(SemanticRuleFixture fixture) : IClassFixture<SemanticRuleFixture>
{
    [Theory]
    ...
    public async Task Counts_concrete_definitions_once(string implementations, string expected)
    {
        ...
        Assert.Equal(expected, string.Join(",", findings.Select(finding => finding.Rule + ":" + finding.Text)));
    }

    [Fact]
    public async Task Constructed_generic_interfaces_and_partial_declarations_count_definitions_once()
    {
        var project = fixture.Project("partial interface I<T> { } partial interface I<T> { } class C : I<string>, I<int> { }");
    ...
    {
        var project = fixture.Project("interface I { }", extra:
            [new DocumentSnapshot("Generated.g.cs", "class C : I { string Value => string.Empty; }", true)]);
    ...
    public async Task Source_dependency_is_counted_and_test_implementations_are_excluded()
    {
        ...
        var tests = fixture.Project("class Fake : I { }", "Tests", isTest: true, dependencies: [contracts]);
    ...
    public async Task Alternate_target_frameworks_have_independent_counts()
    {
        ...
        var second = fixture.Project("interface I { } class C : I { } class D : I { }", framework: "net10.0");
        ...
        Assert.Equal(["DP1003"], response.Contexts.SelectMany(context => context.Findings).Select(finding => finding.RuleId));
    }
    [Fact]
    ...
    public async Task Equal_consumer_frameworks_cannot_union_alternate_dependency_contexts(string extraSource, string expected)
    {
        var contracts = fixture.Project("public interface I { }", "Contracts");
        var first = fixture.Project("public class Implementation : I { }", "Dependency", framework: "net9.0", dependencies: [contracts]);
        var second = fixture.Project("public class Implementation : I { }", "Dependency", framework: "net10.0", dependencies: [contracts]);
        ...
        var response = await fixture.Evaluate(contracts, first, second, firstRoot, secondRoot, extra);

        Assert.Equal(expected, string.Join(",", response.Contexts.SelectMany(context => context.Findings).Select(finding => finding.RuleId)));
    }
}
