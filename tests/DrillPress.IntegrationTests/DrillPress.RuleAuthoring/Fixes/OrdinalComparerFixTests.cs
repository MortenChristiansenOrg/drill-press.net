    ...
    : IClassFixture<SemanticRuleFixture>
{
    ...
    )
    {
        var project = fixture.Project(
            $"using System.Linq; class C {{ object M(string[] values) => {call}; }}"
        );

        var findings = await fixture.Describe(project);

        Assert.Equal(
            [("DP1005", 1, "System.StringComparer.Ordinal", (string?)replacement)],
            findings
        );
    }
    ...
    public async Task Unknown_optional_params_constructor_and_ordering_apis_are_not_fixed(
        string call
    )
    {
        var project = fixture.Project(
    ...
    {
        var project = fixture.Project(
            "using System.Linq; class C { System.Linq.Expressions.Expression<System.Func<string[], object>> M() => values => values.Distinct(System.StringComparer.Ordinal); }"
        );
    ...
    {
        var project = fixture.Project(
            "class C { object M() => Run(() => { var comparer = System.StringComparer.Ordinal; return comparer; }); object Run(System.Func<object> action) => action(); }"
        );
