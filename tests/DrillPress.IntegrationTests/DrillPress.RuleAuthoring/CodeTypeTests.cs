...
public sealed class CodeTypeTests(SemanticRuleFixture fixture) : IClassFixture<SemanticRuleFixture>
{
    ...
    {
        var project = fixture.Project("class C { System.Collections.Generic.List<string> Names = new(); System.Collections.Generic.List<int> Numbers = new(); }");
        ...
        var matches = new[] { identity.Matches(names), identity.Matches(numbers), CodeType.Of<Dictionary<string, int>>().Matches(names) };
    ...
    public void Named_identity_can_restrict_the_declaring_assembly()
    {
        ...
        var matches = new[] { CodeType.Named("Target.Value").Matches(symbol), CodeType.Named("Target.Value", "Product").Matches(symbol),
    ...
    {
        var project = fixture.Project("class C { System.Collections.Generic.List<string>.Enumerator Value; }");
        var symbol = (INamedTypeSymbol)((IFieldSymbol)project.Compilation.GetTypeByMetadataName("C")!.GetMembers("Value").Single()).Type;

        var matches = new[] { CodeType.Of<List<string>.Enumerator>().Matches(symbol), CodeType.Of<List<int>.Enumerator>().Matches(symbol),
            CodeType.Named("System.Collections.Generic.List`1+Enumerator", "System.Unrelated").Matches(symbol) };
    ...
    {
        var project = fixture.Project("delegate void D(); enum E { Value } partial class C { } partial class C { } record R;");
        var rules = new RuleSet();
        rules.For(Code.Types).Forbid("TYPE001", "Review type.");

        var diagnostics = rules.Evaluate(new AnalysisSolution([project]));

        Assert.Equal(["D", "E", "C", "R"], diagnostics.Select(diagnostic => project.Snapshot.Documents[0].Text.Substring(
            diagnostic.Location.Start, diagnostic.Location.Length)));
    }
}
