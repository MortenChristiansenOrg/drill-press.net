...
public sealed class SourceQueryTests(SdkFixture fixture) : IClassFixture<SdkFixture>
{
    ...
    public void Error_types_do_not_match_configured_semantic_identities()
    {
        ...
        var node = Sources
            ...
            .Single();
        var type = node
            .Source.Model.GetTypeInfo(node.Syntax.Type, TestContext.Current.CancellationToken)
            .Type!;
    ...
    public void Reference_search_matches_alias_calls_and_excludes_same_named_parameters()
    {
        ...
        workspace.AddProject(
            ...
            [
                new(
                    "A.cs",
                    "using Alias = Target; class Target { public static void Read() { } } class Caller { void M(int Read) { Alias.Read(); Target.Read(); _ = Read; } }"
                ),
            ]
        );
    ...
    public void Custom_roots_report_each_attribute_with_exact_source_membership()
    {
        ...
        workspace.AddProject(
            "Library",
            [
                new("A.cs", "[System.Obsolete] class A { }"),
        ...
        Assert.Equal(
            [new SourceLocation("A.cs", 1, 15, 1, 2)],
            findings.Select(finding => finding.Location)
        );
    ...
    public void Declarations_include_properties_parameters_locals_and_partial_occurrences()
    {
        ...
        workspace.AddProject(
            ...
            [
                new(
                    "A.cs",
                    "partial class A { public int Value { get; set; } void M(int count) { var local = count; } }"
                ),
        ...
        Assert.Equal(
            ["A", "Value", "count", "local", "A"],
            declarations.Select(item => item.Symbol.Name)
        );
    }
    ...
    public void Syntax_and_constants_remain_available_in_erroneous_source()
    {
        ...
        workspace.AddProject(
            "Library",
            [new("A.cs", "class A { string M() => Missing(\"value\"); }")],
            allowErrors: true
        );
