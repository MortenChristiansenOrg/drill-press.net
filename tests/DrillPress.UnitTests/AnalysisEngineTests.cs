...
public sealed class AnalysisEngineTests
{
    ...
    [Fact]
    public async Task Rule_application_uses_clean_findings_and_failure_exit_codes()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var cleanSnapshot = await WriteSnapshotAsync(
            Path.Combine(Path.GetTempPath(), "drillpress-tests", "Clean.cs"),
            "public static class Clean { public static string Value => \"\"; }",
            cancellationToken);
        var violatingSnapshot = await WriteSnapshotAsync(
        ...
        try
        {
            ...
            Assert.Equal(0, await RuleApplication.RunAsync(
                SampleRuleSet.Create(), ["check", cleanSnapshot], output, TextWriter.Null, cancellationToken));
            Assert.Equal(string.Empty, output.ToString());

            output = new StringWriter();
            Assert.Equal(1, await RuleApplication.RunAsync(
                SampleRuleSet.Create(), ["check", violatingSnapshot], output, TextWriter.Null, cancellationToken));
            Assert.Contains("DP1004", output.ToString(), StringComparison.Ordinal);

            Assert.Equal(2, await RuleApplication.RunAsync(
                SampleRuleSet.Create(), ["check"], TextWriter.Null, TextWriter.Null, cancellationToken));
        }
        finally
        {
            File.Delete(cleanSnapshot);
            File.Delete(violatingSnapshot);
        }
    }
