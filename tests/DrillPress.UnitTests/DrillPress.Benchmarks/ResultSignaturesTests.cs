...
public sealed class ResultSignaturesTests
{
    ...
    public async Task Comparison_detects_different_context_proofs_hidden_by_identical_aggregation()
    {
        ...
        var first = fixture.Response with
        {
            Batches =
            [
                fixture.Response.Batches[0] with
                {
                    Validations = [new("first", false), new("second", true)],
                },
            ],
        };
        var second = fixture.Response with
        {
            Batches =
            [
                fixture.Response.Batches[0] with
        ...
        var expected = await signatures.WriteAsync(
            "snapshot",
            "first",
            "out/first",
            TestContext.Current.CancellationToken
        );
        var actual = await signatures.WriteAsync(
            "snapshot",
            "second",
            "out/second",
            TestContext.Current.CancellationToken
        ...
        Assert.Equal(
            BundleResponseProtocol.Serialize(first),
            fileSystem.File.ReadAllBytes(expected.ResponsePath)
        );
        Assert.Equal(
            BundleResponseProtocol.Serialize(second),
            fileSystem.File.ReadAllBytes(actual.ResponsePath)
        );
    }
    ...
    public async Task Validates_response_association_before_normalizing_run_identity()
    {
        ...
        fileSystem.AddFile(
            "response",
            new(BundleResponseProtocol.Serialize(fixture.Response with { RequestId = "foreign" }))
        );
        var signatures = new ResultSignatures(fileSystem);

        await Assert.ThrowsAsync<InvalidDataException>(() =>
            signatures.WriteAsync(
                "snapshot",
                "response",
                "out/result",
                TestContext.Current.CancellationToken,
