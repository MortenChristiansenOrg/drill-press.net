...
{
    public DocumentSnapshot Document { get; } = SourceIdentity.Capture(
        new DocumentSnapshot("Shared.cs", "😀alpha\r\nbeta gamma", false) { DocumentId = "document" },
        Encoding.UTF8.GetBytes("😀alpha\r\nbeta gamma"), "utf-8", false);
    ...
    public ContractFixture()
    {
        ...
        {
            ContextId = "first", TargetFramework = "net10.0", Documents = [Document], ReferencedContextIds = ["second"],
        };
        var second = first with
        {
            ContextId = "second", TargetFramework = "net9.0", Documents = [Document with { DocumentId = "linked" }], ReferencedContextIds = [],
        };
        Snapshot = CompilationSnapshot.Create(first, second) with { RequestId = "request" };
        Response = new BundleResponse(1, "request",
            [new("first", true, [new("DP1004", "Replace alpha.", "document", 2, 5, "fix")]), new("second", true, [])],
