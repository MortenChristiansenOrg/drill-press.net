...
internal sealed class FixFixture
{
    ...
    public FixFixture(string encodingName = "utf-8", bool bom = false, string replacement = "é", string text = "😀alpha\r\nbeta\ngamma\r", int fileCount = 2)
    {
        var encoding = Encoding.GetEncoding(encodingName);
        Documents = Enumerable.Range(0, fileCount).Select(index => $"{(char)('A' + index)}.cs").Select(name =>
        {
            ...
            return SourceIdentity.Capture(new DocumentSnapshot(path, text, false), bytes, encodingName, bom);
        }).ToArray();
        ...
        var batches = Documents.Select((document, index) => new FixBatch($"fix{index}",
            [new(document.FileIdentity, document.Fingerprint, text.IndexOf("alpha", StringComparison.Ordinal), 5, "alpha", replacement)], [new(project.ContextId, true)])).ToArray();
        Response = new(1, Snapshot.RequestId,
            [new(project.ContextId, true, Documents.Select((document, index) =>
                new Finding("DP1004", "Replace alpha.", document.DocumentId, text.IndexOf("alpha", StringComparison.Ordinal), 5, $"fix{index}")).ToArray())], batches);
