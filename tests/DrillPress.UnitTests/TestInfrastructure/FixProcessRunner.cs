...
internal sealed class FixProcessRunner(FixFixture fixture) : ChildProcessRunner
{
    ...
    )
    {
        ...
        {
            _exports++;
            if (_exports == 2 && FailRegeneration)
                return new(2, "", "regeneration failed\n");
            if (_exports == 2)
            {
                                ...
                                Documents = project
                                    .Documents.Select(document =>
                                        SourceIdentity.Capture(
                                            document with
                                            {
                                                Text = _fixture.FileSystem.File.ReadAllText(
                                                    document.Path
                                                ),
                                            },
                                            _fixture.FileSystem.File.ReadAllBytes(document.Path),
            ...
            await new CompilationSnapshotFile(_fixture.FileSystem).WriteAsync(
                arguments[2],
                _snapshot,
                cancellationToken
            );
        ...
        if (WithholdFixes)
            ...
            {
                Batches = response
                    .Batches.Select(batch => batch with { Validations = [] })
                    .ToArray(),
            };
        return new(
            response.Contexts.Any(context => context.Findings.Length > 0) ? 1 : 0,
