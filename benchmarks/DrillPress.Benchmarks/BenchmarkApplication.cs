...
public static class BenchmarkApplication
{
        ...
        try
        {
            if (args is ["--memory-worker", var planPath, var mode, var caseName])
            {
                var sample = await MemoryProbe.RunWorkerAsync(
                    planPath, Enum.Parse<BundleMode>(mode), caseName);
    ...
    private static async Task MeasureAsync(VerificationSession session)
    {
        ...
        var config = ManualConfig.Create(DefaultConfig.Instance)
            ...
            .WithArtifactsPath(Path.Combine(session.OutputDirectory, "BenchmarkDotNet"));
