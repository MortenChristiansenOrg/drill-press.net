...
public static class BundleResponseProtocol
{
    ...
    public static byte[] Serialize(BundleResponse response) =>
        JsonSerializer.SerializeToUtf8Bytes(response, CompilationSnapshotJsonContext.Default.BundleResponse);
    ...
    {
        var response = JsonSerializer.Deserialize(json, CompilationSnapshotJsonContext.Default.BundleResponse)
