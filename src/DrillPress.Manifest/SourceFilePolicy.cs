...
public sealed class SourceFilePolicy
{
    ...
    public SourceFilePolicy() : this(new FileSystem(), new FileIdentityProbe()) { }
    ...
    public IReadOnlyDictionary<string, SourceFileState> Inspect(CompilationSnapshot snapshot, CancellationToken cancellationToken = default)
    {
        ...
        var aliases = states.Where(pair => pair.Value.Identity is not null).GroupBy(pair => pair.Value.Identity!.Key)
            .Where(group => group.Count() > 1).SelectMany(group => group.Select(pair => pair.Key)).ToArray();
    ...
    public SourceFileState Inspect(string path)
    {
        ...
        try
        {
            ...
            var editable = identity is { LinkCount: 1, IsRegularFile: true } && _fileSystem.File.Exists(path) &&
                (_fileSystem.File.GetAttributes(path) & FileAttributes.ReadOnly) == 0 && !HasSymbolicAlias(path);
    ...
    public CompilationSnapshot Restrict(CompilationSnapshot snapshot, CancellationToken cancellationToken = default)
    {
        ...
        return snapshot with
        {
            Projects = snapshot.Projects.Select(project => project with
            {
                ...
                {
                    IsEditable = document.IsEditable && known && states.TryGetValue(document.FileIdentity, out var state) && state.IsEditable,
                }).ToArray(),
            }).ToArray(),
        };
    }
