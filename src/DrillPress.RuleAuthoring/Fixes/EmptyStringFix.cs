...
public static class EmptyStringFix
{
    ...
    {
        var sources = project
            .Sources.Where(source => source.Document.FileIdentity == edit.FileIdentity)
            .ToArray();
    ...
    private static bool Validate(AnalysisSource source, SourceEdit edit)
    {
        ...
        var expression =
            source.Tree.GetRoot().FindNode(span, getInnermostNodeForTie: true) as ExpressionSyntax;
        return expression is not null
            && expression.Span == span
            && expression.ToString() == edit.OriginalText
