...
{
    private static readonly SymbolDisplayFormat _signature = SymbolDisplayFormat
    ...
    )
    {
        ...
        var changedTree = source.Tree.WithChangedText(
            originalText.WithChanges(new TextChange(replaced.Span, replacement))
        );
        var changedCompilation = source.Project.Compilation.ReplaceSyntaxTree(
            source.Tree,
            changedTree
        );
        if (source.Project.Compilation.Options.SyntaxTreeOptionsProvider is { } provider)
        {
            changedCompilation = changedCompilation.WithOptions(
                changedCompilation.Options.WithSyntaxTreeOptionsProvider(
        ...
        if (
            ...
            || changedModel
                .GetDiagnostics(
                    new TextSpan(scope.SpanStart, scope.Span.Length + delta),
        ...
        foreach (var expression in replaced.AncestorsAndSelf().OfType<ExpressionSyntax>())
        {
            ...
            var rewritten =
                changedTree.GetRoot().FindNode(newSpan, getInnermostNodeForTie: true)
                as ExpressionSyntax;
            if (
                rewritten is null
                || rewritten.Span != newSpan
                || !SameTypeAndConversion(source.Model, expression, changedModel, rewritten)
            )
            {
                return false;
            }
            ...
            else if (expression != replaced)
            {
                ...
                if (
                    before.CandidateReason != CandidateReason.None
                    || after.CandidateReason != CandidateReason.None
    ...
    private static bool SameTypeAndConversion(
    ...
    )
    {
        ...
        return oldType.Type?.TypeKind != TypeKind.Error
            && newType.Type?.TypeKind != TypeKind.Error
            && Identity(oldType.Type) == Identity(newType.Type)
            && Identity(oldType.ConvertedType) == Identity(newType.ConvertedType)
