...
public sealed class TestBody
{
    internal TestBody(CodeMethod method)
    {
        ...
        excluded.AddRange(
            body.DescendantTrivia(descendIntoTrivia: true)
                .Where(trivia =>
        ...
        EmptyLines = method
            .Source.Tree.GetText()
            .Lines.Where(line =>
                line.Start > body.OpenBraceToken.Span.End
                && line.End < body.CloseBraceToken.Span.Start
                && string.IsNullOrWhiteSpace(line.ToString())
                && !excluded.Any(span => span.IntersectsWith(line.SpanIncludingLineBreak))
            )
    ...
    public SourceLocation? EarlyAssertion =>
        EmptyLines.Count == 0 || Assertions is [{ MemberName: "Throws" }]
            ? null
            : Assertions
                .FirstOrDefault(assertion => assertion.Location.Start < EmptyLines[^1].Start)
                ?.Location;

    private static TextSpan? NestedBody(SyntaxNode node) =>
        node switch
        {
            LocalFunctionStatementSyntax local => local.Body?.Span
                ?? local.ExpressionBody?.Expression.Span,
