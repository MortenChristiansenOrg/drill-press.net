...
internal sealed class MemberCandidateIndex
{
    ...
    internal MemberCandidateIndex(IEnumerable<AnalysisSource> sources, CancellationToken cancellationToken)
    {
    ...
    private MemberReference? Bind(MemberSyntaxCandidate candidate)
    {
        ...
        var symbol = candidate.Source.Model.GetSymbolInfo(candidate.Syntax, candidate.Source.Project.CancellationToken).Symbol;
        if (symbol is not ((IFieldSymbol or IPropertySymbol or IMethodSymbol) and { ContainingType: { IsAnonymousType: false } }))
        {
            return null;
        }

        return new MemberReference(CodeType.FromSymbol(symbol.ContainingType), symbol.Name, candidate.Source.Locate(candidate.Syntax.Span))
        {
