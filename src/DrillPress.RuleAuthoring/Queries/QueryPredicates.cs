...
public static class QueryPredicates
{
    ...
    public static CodeQuery<T> Where<T>(this CodeQuery<T> query, Func<T, bool> predicate) => query.Where(new RuleCondition<T>(predicate));

    /// <summary>Removes an explicitly configured exception from a selection.</summary>
    public static CodeQuery<T> ExceptWhen<T>(this CodeQuery<T> query, Func<T, bool> predicate) => query.ExceptWhen(new RuleCondition<T>(predicate));

    /// <summary>Reports candidates that do not satisfy a readable requirement.</summary>
    public static RuleScope<T> Require<T>(this RuleScope<T> scope, Func<T, bool> predicate, string id, string message,
