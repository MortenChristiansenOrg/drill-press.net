...
internal sealed class CompilationViews(AnalysisSolution solution)
{
    ...
    public bool Reaches(AnalysisProject project, string contextId) => Closure(project)
        .Any(dependency => dependency.Snapshot.ContextId == contextId);
    ...
    public AnalysisProject[][] For(AnalysisProject owner)
    {
        ...
        var consumers = solution.Projects.Where(project => Reaches(project, owner.Snapshot.ContextId)).ToArray();
        var roots = consumers.Where(project => !consumers.Any(other => other != project && Reaches(other, project.Snapshot.ContextId))).ToArray();
    ...
    private void Expand(List<AnalysisProject> selected, List<AnalysisProject> candidates,
        List<AnalysisProject> excluded, List<AnalysisProject[]> result)
    {
        ...
        var pivot = candidates.Concat(excluded)
            .MaxBy(project => candidates.Count(candidate => candidate != project && Compatible(project, candidate)));
        var choices = candidates.Where(candidate => candidate == pivot || pivot is null || !Compatible(candidate, pivot)).ToArray();
        foreach (var candidate in choices)
        {
            Expand([.. selected, candidate], candidates.Where(other => other != candidate && Compatible(candidate, other)).ToList(),
                excluded.Where(other => Compatible(candidate, other)).ToList(), result);
    ...
    private bool Compatible(AnalysisProject left, AnalysisProject right)
    {
        ...
        return first.All(project => second.All(other => project.Snapshot.ProjectPath != other.Snapshot.ProjectPath ||
    ...
    private AnalysisProject[] Closure(AnalysisProject root)
    {
        ...
        while (pending.TryPop(out var project))
        {
            ...
            {
                pending.Push(solution.Projects.Single(candidate => candidate.Snapshot.ContextId == id));
            }
        }
