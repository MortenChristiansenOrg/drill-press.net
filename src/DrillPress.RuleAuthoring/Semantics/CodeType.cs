...
public readonly record struct CodeType(string MetadataName)
{
    ...
    public bool Matches(INamedTypeSymbol symbol)
    {
        ...
        return MetadataName == actual.MetadataName
            ...
            && (
                ...
                || AssemblyName == symbol.ContainingAssembly.Identity.ToString()
    ...
    public bool Matches(ITypeSymbol symbol)
    {
        ...
        if (symbol is IArrayTypeSymbol array)
        {
            ...
            return MetadataName.EndsWith(suffix, StringComparison.Ordinal)
                && (this with { MetadataName = MetadataName[..^suffix.Length] }).Matches(
                    array.ElementType
                );
        }
    ...
    internal static CodeType FromSymbol(INamedTypeSymbol symbol) =>
        new(MetadataNameOf(symbol))
        {
    ...
    internal static string MetadataNameOf(INamedTypeSymbol symbol) =>
        symbol.ContainingType is { } parent ? $"{MetadataNameOf(parent)}+{symbol.MetadataName}"
    ...
    private static string SymbolArgument(ITypeSymbol symbol) =>
        symbol switch
        {
            INamedTypeSymbol named => MetadataNameOf(named)
                + "@"
                + (IsFrameworkSymbol(named) ? "framework" : named.ContainingAssembly.Identity.Name)
                + (
                    named.IsGenericType
                        ? "["
            ...
            IArrayTypeSymbol array => SymbolArgument(array.ElementType)
                + "["
    ...
    private static CodeType FromRuntime(Type type) =>
        type.IsArray
    ...
    private static string RuntimeArgument(Type type) =>
        type.IsArray
            ? RuntimeArgument(type.GetElementType()!)
                + "["
                + new string(',', type.GetArrayRank() - 1)
                + "]"
            : RuntimeName(type)
                + "@"
                + RuntimeAssembly(type)
                + (
    ...
    private static IEnumerable<ITypeSymbol> AllTypeArguments(INamedTypeSymbol type) =>
        (type.ContainingType is { } parent ? AllTypeArguments(parent) : []).Concat(
            type.TypeArguments
        );
    ...
    private static string RuntimeName(Type type)
    {
        if (type.IsArray)
        {
            return RuntimeName(type.GetElementType()!)
                + "["
                + new string(',', type.GetArrayRank() - 1)
                + "]";
        }
