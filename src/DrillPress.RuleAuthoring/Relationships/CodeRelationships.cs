...
public sealed class CodeRelationships
{
    ...
    private static readonly CodeQuery<CodeInvocation> _semanticCalls =
        OperationQueries.InvocationsIn(Sources.FilesIncludingGenerated);
    ...
    private Dictionary<DeclarationKey, List<CodeInvocation>> BuildCalls()
    {
        ...
        {
            var owner = call.Source.Model.GetEnclosingSymbol(
                call.Operation.Syntax.SpanStart,
                _solution.CancellationToken
            );
    ...
    public static CodeRelationships In(AnalysisSolution solution) =>
        solution.Cached(_cacheKey, () => new CodeRelationships(solution));
    ...
    public IReadOnlyList<CodeInvocation> CallsFrom(IMethodSymbol method) =>
        Key(method) is { } key && _calls.Value.TryGetValue(key, out var calls)
            ? calls.AsReadOnly()
            : [];

    /// <summary>Finds source calls bound to this declaration; metadata-only targets have no declaration key.</summary>
    public IReadOnlyList<CodeInvocation> CallersOf(IMethodSymbol method) =>
        Key(method) is { } key
            ? _calls
                .Value.Values.SelectMany(calls => calls)
                .Where(call => Key(call.Target) == key)
                .ToArray()
            : [];
    ...
    public IReadOnlyList<CodeDeclaration> DerivedTypes(CodeDeclaration declaration)
    {
        var compatible = _views
            ...
            .SelectMany(view => view)
            .ToHashSet();
        return _solution
            .Types.Where(candidate =>
                compatible.Contains(candidate.Source.Project)
                && Ancestors(candidate.Symbol)
                    .Any(symbol => SameDeclaration(symbol, declaration.Symbol))
            )
            .ToArray();
    }
    ...
    public IReadOnlyList<AnalysisSource> FilesOf(CodeDeclaration declaration) =>
        declaration
            .Source.Project.Sources.Where(source =>
                ...
                && declaration.Symbol.DeclaringSyntaxReferences.Any(reference =>
    ...
    private static bool SameDeclaration(INamedTypeSymbol left, INamedTypeSymbol right) =>
        left.OriginalDefinition.DeclaringSyntaxReferences.Any(first =>
            right.OriginalDefinition.DeclaringSyntaxReferences.Any(second =>
                first.SyntaxTree == second.SyntaxTree && first.Span == second.Span
            )
        );
