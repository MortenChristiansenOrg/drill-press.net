...
public static class XunitTests
{
    ...
    public static RuleCondition<CodeMethod> AreTests { get; } = new(method =>
        method.Symbol?.GetAttributes().Any(attribute => IsTestAttribute(attribute.AttributeClass)) == true);
    ...
    internal static bool IsXunitType(INamedTypeSymbol type, string metadataName) =>
        CodeType.MetadataNameOf(type) == metadataName && type.ContainingAssembly.Name is
            "xunit.core" or "xunit.v3.core" or "xunit.assert" or "xunit.v3.assert";

    private static bool IsTestAttribute(INamedTypeSymbol? type)
    {
        for (var current = type; current is not null; current = current.BaseType)
        {
            if (IsXunitType(current, "Xunit.FactAttribute") || IsXunitType(current, "Xunit.TheoryAttribute"))
            {
                return true;
            }
        }
