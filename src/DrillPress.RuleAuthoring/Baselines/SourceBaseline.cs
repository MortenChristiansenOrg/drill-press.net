...
public sealed class SourceBaseline
{
    ...
    private readonly Dictionary<(string Project, string Symbol), (Accessibility Accessibility, string Text)> _symbols = [];
    ...
    public SourceBaseline(AnalysisSolution accepted)
    {
        foreach (var project in accepted.Projects)
        {
            ...
            foreach (var source in project.Sources.Where(source => !source.Document.IsGenerated))
            {
                ...
                foreach (var syntax in source.Tree.GetRoot(accepted.CancellationToken).DescendantNodes())
                {
                    ...
                    {
                        _symbols[(key, id)] = (symbol.DeclaredAccessibility, DeclarationText(symbol, accepted.CancellationToken));
                    }
                }
            }
        }
    ...
    public SourceChange ChangeOf(CodeFile file) =>
        !_files.TryGetValue((ProjectKey(file.Source.Project), Normalize(file.Source.Document.Path)), out var text)
            ? SourceChange.Added : text == file.Source.Document.Text ? SourceChange.Unchanged : SourceChange.Modified;
    ...
    public Accessibility? PreviousAccessibility(AnalysisProject project, ISymbol symbol) =>
        symbol.GetDocumentationCommentId() is { } id && _symbols.TryGetValue((ProjectKey(project), id), out var previous)
            ? previous.Accessibility : null;
    ...
    public SourceChange ChangeOf(AnalysisProject project, ISymbol symbol) =>
        symbol.GetDocumentationCommentId() is not { } id || !_symbols.TryGetValue((ProjectKey(project), id), out var previous)
            ? SourceChange.Added : previous.Text == DeclarationText(symbol, project.CancellationToken) ? SourceChange.Unchanged : SourceChange.Modified;

    /// <summary>Restricts current files to additions or text changes against this baseline.</summary>
    public CodeQuery<CodeFile> ChangedFiles => Sources.Files.Where(new(file => ChangeOf(file) != SourceChange.Unchanged));

    private static string Normalize(string path) => path.Replace('\\', '/');

    private static string DeclarationText(ISymbol symbol, CancellationToken cancellationToken) => string.Join("\n",
        symbol.DeclaringSyntaxReferences.OrderBy(reference => reference.SyntaxTree.FilePath, StringComparer.Ordinal).ThenBy(reference => reference.Span.Start)
            .Select(reference => reference.GetSyntax(cancellationToken).ToString()));

    private static string ProjectKey(AnalysisProject project) => Normalize(project.Snapshot.ProjectPath) + "|" +
        project.Snapshot.TargetFramework + "|" + string.Join("|", project.Snapshot.Properties.OrderBy(pair => pair.Key, StringComparer.OrdinalIgnoreCase)
            .Select(pair => $"{pair.Key.Length}:{pair.Key.ToUpperInvariant()}{pair.Value.Length}:{pair.Value}"));
}
