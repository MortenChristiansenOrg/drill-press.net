...
public static class ProjectFacts
{
    ...
    public static CodeQuery<CodeFile> TestFiles { get; } =
        Sources.Files.Where(new(file => file.Source.Project.Snapshot.IsTestProject));

    /// <summary>Restricts files to contexts not classified as test projects.</summary>
    public static CodeQuery<CodeFile> ProductionFiles { get; } =
        Sources.Files.Where(new(file => !file.Source.Project.Snapshot.IsTestProject));
    ...
    public static bool ReferencesPackage(this AnalysisProject project, string packageId)
    {
        ...
        return project.Snapshot.Packages.Any(package =>
    ...
    public static CodeQuery<AnalysisProject> WithRole(Func<AnalysisProject, bool> role) =>
        Sources.Projects.Where(new(role));
}
