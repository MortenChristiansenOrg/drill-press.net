...
internal sealed class TargetResolver(IFileSystem fileSystem)
{
    ...
    public string Resolve(string target)
    {
        ...
        if (_fileSystem.Directory.Exists(path))
        {
            ...
            var candidates =
                solutions.Length > 0
                    ? solutions
                    : files.Where(file => HasExtension(file, ".csproj")).ToArray();
        ...
        if (
            !_fileSystem.File.Exists(path) || !HasExtension(path, ".csproj", ".sln", ".slnx", ".cs")
        )
        {
            throw new FileNotFoundException(
                $"C# target '{target}' was not found or has an unsupported extension.",
                path
            );
        }
    ...
    public string[] ExpandSources(string target)
    {
        ...
        return matches.Length > 0
            ? matches
            : throw new FileNotFoundException($"C# glob '{target}' matched no files.");
    }
