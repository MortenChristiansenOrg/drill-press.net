...
internal sealed class LooseSourceLoader(IFileSystem fileSystem)
{
    ...
    public Task<SnapshotExport> LoadAsync(
        string[] paths,
    ...
    )
    {
        if (options.Properties.Count > 0)
        {
            throw new ArgumentException(
                "MSBuild properties apply only to solution and project targets."
            );
        }
        ...
        var trees = paths
            .Select(path =>
            {
                ...
                return CSharpSyntaxTree.ParseText(
                    SourceText.From(stream, System.Text.Encoding.UTF8),
        ...
        var referencePaths =
            ...
            ?? throw new InvalidOperationException(
                "The BuildHost runtime did not provide its platform assemblies."
            );
        ...
        return Task.FromResult(
            new SnapshotExport(
                CompilationSnapshot.Create(snapshot),
                [new CompilationContext(snapshot, compilation)]
            )
        );
    }
}
